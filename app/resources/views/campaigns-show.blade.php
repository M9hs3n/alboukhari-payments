...
<div style="max-width:1200px;margin:0 auto">
    <div class="page-header">
        ...
        <a href="{{ route('campaigns.index') }}" class="btn">← {{ __('Back to log') }}</a>
    </div>
    ...
    <div class="page-card">
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:12px;font-size:12px">
            <div><span class="text-muted">{{ __('Period') }}:</span> <strong>{{ $campaign->period_year }}/{{ str_pad($campaign->period_month, 2, '0', STR_PAD_LEFT) }}</strong></div>
            <div><span class="text-muted">Tag:</span> <code>{{ $campaign->tag }}</code></div>
            <div><span class="text-muted">{{ __('Started') }}:</span> {{ $campaign->started_at?->format('Y-m-d H:i') ?? '—' }}</div>
            <div><span class="text-muted">{{ __('Finished') }}:</span> {{ $campaign->finished_at?->format('Y-m-d H:i') ?? '—' }}</div>
        </div>

        <h3 class="mt-4">{{ __('Original message') }}</h3>
    ...
    <div class="page-card" style="padding:0">
        <h3 style="margin:0;padding:16px 20px;border-bottom:1px solid var(--color-border)">{{ __('Recipients') }}</h3>
        <table class="students-grid" style="font-size:12px">
            <thead>
                <tr>
                    <th style="padding:8px">#</th>
                    <th style="text-align:start">{{ __('Recipient') }}</th>
                    <th>{{ __('columns.phone') }}</th>
                    <th>📲 SMS</th>
                    <th>{{ __('columns.status') }}</th>
                    <th>{{ __('Cost') }}</th>
                    <th style="text-align:start">{{ __('Last error') }}</th>
                </tr>
            </thead>
            <tbody>
